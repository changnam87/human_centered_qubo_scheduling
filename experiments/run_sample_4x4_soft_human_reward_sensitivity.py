...
def solve_with_human_reward(instance, human_reward):
    ...
    # ------------------------------------------------------------
    # Objective with soft human reward
    #
    # Important:
    #   CP-SAT minimizes the objective.
    #   Therefore, human reward is subtracted from the coefficient
    #   when a human worker is assigned.
    # ------------------------------------------------------------
    SCALE = 10
    objective_terms = []
    ...
    row = {
        ...
        "ergonomic": None,
        "safety": None,
        "original_cost_without_reward": None,
        "reward_value": None,
        "reward_adjusted_objective": None,
        "total_penalty": None,
        "total_cost_without_reward": None,
        "schedule": None
    }
    ...
    total_cost_without_reward = result["total_cost"]
    reward_value = human_reward * human_count
    reward_adjusted_objective = total_cost_without_reward - reward_value
    ...
    row.update({
        ...
        "original_cost_without_reward": result["original_cost"],
...
def main():
    ...
    output_path = PROJECT_ROOT / "results" / "tables" / "sample_4x4_soft_human_reward_sensitivity.csv"
    ...
    print("=== sample_4x4 soft human-reward sensitivity ===")
    print("Instance:", instance_path)
    print("Human reward values:", reward_values)

    for human_reward in reward_values:
        print()
        print("=" * 70)
        print("Solving human_reward =", human_reward)
        print("=" * 70)

        row = solve_with_human_reward(
            instance,
            human_reward=human_reward
        )
        ...
        print("Total cost without reward:", row["total_cost_without_reward"])
        print("Reward value:", row["reward_value"])
        print("Reward-adjusted objective:", row["reward_adjusted_objective"])
    ...
    baseline_cost = float(df[df["human_reward"] == 0].iloc[0]["total_cost_without_reward"])
    baseline_objective = float(df[df["human_reward"] == 0].iloc[0]["reward_adjusted_objective"])

    df["cost_increase_vs_reward0"] = df["total_cost_without_reward"] - baseline_cost
    df["percent_cost_increase_vs_reward0"] = df["cost_increase_vs_reward0"] / baseline_cost * 100.0

    df["objective_change_vs_reward0"] = df["reward_adjusted_objective"] - baseline_objective

    df.to_csv(output_path, index=False)

    print()
    print("=== Soft human-reward sensitivity summary ===")
    print(df[[
        "human_reward",
        "status",
        "feasible",
        ...
        "cost_increase_vs_reward0",
        "percent_cost_increase_vs_reward0",
        "objective_change_vs_reward0"
    ]])

    print()
    print("Saved soft human-reward sensitivity results to:", output_path)
