...
def main():
    ...
    output_path = PROJECT_ROOT / "results" / "tables" / "sample_4x4_human_utilization_cpsat_result.csv"

    min_human_assignments = 2

    print("=== sample_4x4 CP-SAT with human-utilization constraint ===")
    print("Input:", instance_path)
    print("Minimum human assignments:", min_human_assignments)
    ...
    print("Created CP-SAT variables:", len(x))
    ...
    #
    # Count how many operations are assigned to any human worker.
    # ------------------------------------------------------------
    human_use_terms = []
    ...
    print()
    print("=== Solving CP-SAT human-utilization variant ===")
    ...
    print("Best objective bound scaled:", solver.BestObjectiveBound())
    print("Best objective bound unscaled:", solver.BestObjectiveBound() / SCALE)
    ...
    # Count human and robot assignments from decoded schedule
    human_assignment_count = 0
    robot_assignment_count = 0

    for operation, selected in result["schedule"].items():
        # The evaluator may return selected assignment either as:
        # 1) {"resource": r, "time": t}
        # 2) [r, t]
        # 3) [(r, t)] or [{"resource": r, "time": t}]
        resource = None
    ...
    print("Robot assignment count:", robot_assignment_count)
    ...
    row = {
        "solver": "CP-SAT",
        "variant": "human_utilization_min_assignments",
        ...
        "robot_assignment_count": robot_assignment_count,
    ...
    print()
    print("Saved human-utilization CP-SAT result to:", output_path)
