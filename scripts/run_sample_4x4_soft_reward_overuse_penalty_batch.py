"""
Batch pilot: soft human reward + over-utilization penalty for sample_4x4.

Purpose
-------
The previous fine-grid pilot showed that a linear soft human reward successfully
induces human involvement, but can become too strong at high reward values.

This script tests whether an over-utilization penalty can encourage human
involvement while preventing excessive assignment of operations to human
resources.
...
For each instance:
    ...
    objective
        = total_cost_without_reward
          ...
          + overuse_penalty * max(0, human_assignment_count - target_human_assignments)
...
DEFAULT_HUMAN_REWARDS = [2.5, 3.0, 3.5]
DEFAULT_OVERUSE_PENALTIES = [0.0, 0.5, 1.0, 2.0, 3.0, 4.0]
...
def generate_sample_4x4_augmented_instance(seed: int) -> Instance:
    ...
    This generator intentionally matches the previous fine-grid batch pilot so
    that this overuse-penalty experiment is directly comparable.
    """
...
def solve_instance(
    ...
    # Each operation starts exactly once on exactly one resource.
    for op in instance.operations:
        model.Add(
            sum(
                x[(op.op_id, r, t)]
    ...
    overuse_count = model.NewIntVar(0, NUM_OPERATIONS, "overuse_count")
    model.Add(overuse_count >= human_count - target_human_assignments)
    model.Add(overuse_count >= 0)

    reward_scaled = int(round(SCALE * human_reward))
    overuse_penalty_scaled = int(round(SCALE * overuse_penalty))

    objective_scaled = (
        total_without_reward_scaled
        - reward_scaled * human_count
        + overuse_penalty_scaled * overuse_count
    )
    ...
    overuse_value = max(0, human_assignments - target_human_assignments)
    ...
    overuse_penalty_term = overuse_penalty * overuse_value

    adjusted_objective_manual = (
        total_cost_without_reward
        - reward_term
        + overuse_penalty_term
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--meta-out",
        type=str,
        default="results/tables/sample_4x4_soft_reward_overuse_penalty_batch_metadata.json",
    )
    ...
    overuse_penalties = parse_float_list(args.overuse_penalties)
    ...
    print("=== sample_4x4 soft reward + overuse penalty batch pilot ===")
    print(f"Seeds: {seeds}")
    print(f"Human rewards: {human_rewards}")
    print(f"Overuse penalties: {overuse_penalties}")
    ...
    for seed in seeds:
        ...
        for human_reward in human_rewards:
            for overuse_penalty in overuse_penalties:
                ...
                print(
                    "reward={reward:>3.1f} | overuse_penalty={penalty:>3.1f} | "
                    "status={status:<8} | humans={humans:>2} | overuse={overuse:>2} | "
                    "dist={dist:>2} | adj_obj={obj:>7.2f} | time={sec:>5.2f}s".format(
                        reward=human_reward,
                        penalty=overuse_penalty,
                        status=result["status"],
                        humans=result["human_assignments"],
                        overuse=result["overuse_count"],
    ...
    meta = {
        "experiment": "sample_4x4_soft_reward_overuse_penalty_batch",
        "seed_start": args.seed_start,
        "seed_end": args.seed_end,
        "seeds": seeds,
        "human_rewards": human_rewards,
        "overuse_penalties": overuse_penalties,
        "target_human_assignments": args.target_human_assignments,
        "time_limit_seconds": args.time_limit,
        "num_workers": args.num_workers,
        "jobs": NUM_JOBS,
        ...
        "note": "Prototype/pilot over-utilization penalty test; not final paper-level evidence.",
    }
