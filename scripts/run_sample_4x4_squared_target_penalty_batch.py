"""
Batch pilot: QUBO-compatible squared target human-utilization penalty.
...
---------
    objective
        = total_cost_without_reward
          ...
          + lambda_target * (human_assignment_count - target_human_assignments)^2

Purpose
-------
This pilot tests a QUBO-compatible target-based human-utilization formulation
on sample_4x4 synthetic augmentation seeds.

The squared target penalty is more natural for QUBO/Ising formulations than an
...
def solve_instance(
    ...
    squared_deviation = model.NewIntVar(0, NUM_OPERATIONS * NUM_OPERATIONS, "squared_deviation")

    allowed_pairs = []
    for d in range(-target_human_assignments, NUM_OPERATIONS - target_human_assignments + 1):
        allowed_pairs.append((d, d * d))

    model.AddAllowedAssignments([diff_from_target, squared_deviation], allowed_pairs)

    reward_scaled = int(round(SCALE * human_reward))
    lambda_target_scaled = int(round(SCALE * lambda_target))

    objective_scaled = (
        total_without_reward_scaled
        - reward_scaled * human_count
        + lambda_target_scaled * squared_deviation
    )
    ...
    squared_deviation_value = distance_from_target**2
    ...
    squared_target_penalty_term = lambda_target * squared_deviation_value
    ...
    return {
        ...
        "squared_target_penalty_term": squared_target_penalty_term,
        ...
        "squared_deviation": squared_deviation_value,
...
def main() -> None:
    ...
    parser.add_argument(
        "--lambda-targets",
        type=str,
        default="0.0,0.1,0.25,0.5,1.0,2.0,3.0",
    )
    ...
    parser.add_argument(
        "--meta-out",
        type=str,
        default="results/tables/sample_4x4_squared_target_penalty_batch_metadata.json",
    )
    ...
    print("=== sample_4x4 squared target penalty batch pilot ===")
    print(f"Seeds: {seeds}")
    ...
    for seed in seeds:
        ...
        for human_reward in human_rewards:
            for lambda_target in lambda_targets:
                ...
                print(
                    "reward={reward:>3.1f} | lambda={lam:>4.2f} | "
                    "status={status:<8} | humans={humans:>2} | "
                    "dist={dist:>2} | sqdev={sqdev:>3} | adj_obj={obj:>7.2f} | time={sec:>5.2f}s".format(
                        reward=human_reward,
                        lam=lambda_target,
                        status=result["status"],
                        humans=result["human_assignments"],
                        dist=result["distance_from_target"],
                        sqdev=result["squared_deviation"],
    ...
    meta = {
        "experiment": "sample_4x4_squared_target_penalty_batch",
        ...
        "note": "Prototype/pilot QUBO-compatible squared target penalty test; not final paper-level evidence.",
    }
