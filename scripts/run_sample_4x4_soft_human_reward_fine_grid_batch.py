"""
Fine-grid soft human-reward batch pilot for sample_4x4.

This script refines the previous integer-grid reward sensitivity:

    human_reward = 0, 1, 2, 3, 4, 5

into a finer grid:

    human_reward = 0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0

Purpose
-------
The previous batch pilot showed that human involvement begins around reward 2-3.
This fine-grid pilot estimates that activation threshold more precisely.

This remains a prototype / pilot validation experiment.
It is not a final paper-level experiment.
"""
...
FINE_REWARD_VALUES = [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
...
def generate_sample_4x4_augmented_instance(seed: int) -> Instance:
    ...
    This generator intentionally matches the previous integer-grid batch pilot
    so that the fine-grid experiment is directly comparable.
    """
...
def solve_soft_reward_instance(
    ...
    # Resource capacity: one operation per resource at each time point.
    for r in range(NUM_RESOURCES):
...
def main() -> None:
    ...
    parser.add_argument(
        "--meta-out",
        type=str,
        default="results/tables/sample_4x4_soft_human_reward_fine_grid_batch_metadata.json",
    )
    ...
    print("=== sample_4x4 fine-grid soft human-reward batch pilot ===")
    print(f"Seeds: {seeds}")
    print(f"Rewards: {FINE_REWARD_VALUES}")
    ...
    for seed in seeds:
        ...
        for reward in FINE_REWARD_VALUES:
            ...
            print(
                "reward={reward:>3.1f} | status={status:<8} | humans={humans:>2} | "
    ...
    meta = {
        "experiment": "sample_4x4_soft_human_reward_fine_grid_batch_sensitivity",
        "seed_start": args.seed_start,
        "seed_end": args.seed_end,
        "seeds": seeds,
        "reward_values": FINE_REWARD_VALUES,
        "time_limit_seconds": args.time_limit,
        "num_workers": args.num_workers,
        "jobs": NUM_JOBS,
        "machines": NUM_MACHINES,
        ...
        "note": "Fine-grid prototype/pilot reward-threshold robustness check; not final paper-level evidence.",
    }
